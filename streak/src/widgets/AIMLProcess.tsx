...
const AIMLProcess = (props: ProcessSectionProps) => {
  const {
  ...
  return (
    <section
      id={id}
      className="relative border-t py-12 xl:py-20 xxl:py-26 bg-contain bg-no-repeat"
    ...
    >
      ...
      <div className="container pointer-events-none">
        {heading && <h1 className="text-xxl xl:text-h4 mb-2">{heading}</h1>}
        {subHeading && (
          <h1 className="text-primary text-h4 xl:text-h2 mb-10 lg:mb-26">
            {subHeading}
          </h1>
        )}
...
const StepCard = ({ item }: { item: StepItem }) => (
  <>
    <div className="relative lg:w-[420px] lg:h-[152px]  rounded-lg max-lg:mb-6 overflow-hidden before:absolute before:right-0 before:top-0 before:w-full before:h-screen before:bg-gradient-process before:z-10">
      <img src={item.img} alt={item.title} className="w-full max-lg:hidden" />
      <img src={item.imgSm} alt={item.title} className="w-full lg:hidden" />

      <span className="absolute left-0 top-0 flex items-center lg:justify-center w-full h-full text-white text-xxl font-semibold p-8 lg:p-4 z-20">
        {item.title}
      </span>
    </div>
