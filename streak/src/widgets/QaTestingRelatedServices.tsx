...
const QaTestingRelatedServices = (props: Props) => {
  const slides = props?.data;

  return (
    <section className="relative text-black bg-white overflow-hidden pb-6 xl:pb-10" id="related_services">
      <div className="container will-change-transform">
        <div className="flex justify-between">
          <h1 className="text-h4 xl:text-h2">Related Services</h1>
        </div>
        <div className="embla mt-6 lg:mt-18">
          <div className="embla__container flex -mx-3 xl:-mx-5 xxl:-mx-7">

            {slides.map((slide: any) => (
              <a href={slide.link} className="embla__slide flex flex-col flex-none w-[94%] md:w-1/3 xl:w-[33.33%] px-3 xl:px-5 xxl:px-7 group" >
                ...
                <div className="flex flex-col justify-between h-full mt-8">
                  <div>
                    <h2 className="text-xxl">{slide.title}</h2>
                    <p className="text-md mt-4">{slide.description}</p>
                  </div>
      ...
      <Script id="QaTestingRelatedServices">
        {(gDom: any) => {
          gDom
            .loadPackage("js/embla-carousel.umd.js")
            .then(() => {
              const section = document.getElementById("related_services");
...
export default QaTestingRelatedServices;